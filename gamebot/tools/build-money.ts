 ...
 *
 * Wilderness spawns are excluded. They are where the valuable ones are, and they
 * are also where an unattended bot gets killed by other players.
 *
 *   bun bots/<name>/tools/build-money.ts > bots/<name>/lib/money/world.generated.ts
 */
...
/** Wilderness starts here; underground wilderness is the same z plus 6400. */
const WILDERNESS_Z = 3520;
...
for (const file of readdirSync(MAPS)) {
    ...
    for (const line of readFileSync(join(MAPS, file), 'utf8').split('\n')) {
        ...
        if (section === 'OBJ') {
            ...
            if (!item) continue;
            const surfaceZ = z > UNDERGROUND_Z_OFFSET ? z - UNDERGROUND_Z_OFFSET : z;
            if (surfaceZ >= WILDERNESS_Z) continue;
...
console.log('export interface Shop { npc: string; title: string; buyMultiplier: number; haggle: number; level: number; x: number; z: number; stock: Record<string, number>; buysAnything: boolean }\n');
